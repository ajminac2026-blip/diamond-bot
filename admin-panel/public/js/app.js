// Check authentication first (only for admin panel page, not login page)
...
async function loadDiamondStatus() {
    try {
        const response = await authFetch('/api/diamond-status');
...
async function toggleDiamondSystem() {
    try {
        ...
        const response = await authFetch('/api/diamond-status/toggle', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function saveGlobalMessage() {
    try {
        ...
        const response = await authFetch('/api/diamond-status/global-message', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function saveStock() {
    try {
        ...
        const response = await authFetch('/api/diamond-status/set-stock', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function saveGroupMessage(groupId) {
    try {
        ...
        const response = await authFetch('/api/diamond-status/group-message', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function loadLastAutoDeduction() {
    try {
        const response = await authFetch('/api/auto-deductions');
        ...
        container.innerHTML = `
            <div class="auto-deduct-item" style="flex-wrap: wrap; gap: 15px;">
                <div class="auto-deduct-item-left" style="flex-basis: 100%; min-width: 200px;">
                ...
                <div class="auto-deduct-item-amount" style="flex-basis: 100%; text-align: center; padding: 10px; border-radius: 8px; background: rgba(245, 87, 108, 0.15);">
...
async function updateGroupRate(groupId) {
    ...
    try {
        const response = await authFetch(`/api/groups/${groupId}/rate`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function updateGroupDueLimit(groupId) {
    ...
    try {
        const response = await authFetch(`/api/groups/${groupId}/due-limit`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function clearGroupData(groupId, groupName) {
    ...
    try {
        const response = await authFetch(`/api/groups/${groupId}/clear`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
...
async function applyBulkRate(groupIds) {
    ...
    try {
        const response = await authFetch('/api/groups/bulk-rate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function loadPendingDeposits() {
    try {
        const response = await authFetch('/api/deposits/pending');
...
async function approveDeposit(depositId) {
    ...
    try {
        const response = await authFetch(`/api/deposits/${depositId}/approve`, {
            method: 'POST'
        });
...
async function rejectDeposit(depositId) {
    ...
    try {
        const response = await authFetch(`/api/deposits/${depositId}/reject`, {
            method: 'POST'
        });
...
function saveUserChanges(phone) {
    ...
    authFetch(`/api/users/${phone}/update`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function toggleUserBlock(phone) {
    try {
        const response = await authFetch(`/api/users/${phone}/toggle-block`, {
            method: 'POST'
        });
        const result = await response.json();
                    ...
                    <div class="modal-content large-modal" onclick="event.stopPropagation()">
                        ...
                        <div class="modal-body">
                            <div style="margin-bottom: 20px;">
                                <button onclick="showAddPaymentModal()" class="btn-primary" style="padding: 10px 20px; border: none; border-radius: 8px; background: #667eea; color: white; cursor: pointer; font-weight: 600; width: 100%;">
                            ...
                            <div style="display: grid; gap: 15px;">
                                ${data.paymentNumbers && data.paymentNumbers.length > 0 ? data.paymentNumbers.map((payment, idx) => `
                                    <div style="background: rgba(255,255,255,0.05); padding: 12px; border-radius: 10px; border-left: 4px solid #667eea;">
                                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px; flex-wrap: wrap; gap: 10px;">
                                            <div style="flex: 1; min-width: 150px;">
                                                <h4 style="margin: 0 0 5px 0; color: #4facfe; font-size: 1rem;">${payment.method}</h4>
                                                <p style="margin: 0; color: #aaa; font-size: 0.85rem;">
                                                    ${payment.isBank ? `<strong>Bank Account</strong>` : '<strong>Mobile Payment</strong>'}
                                                </p>
                                            </div>
                                            <div style="display: flex; gap: 8px;">
                                                <button onclick="deletePaymentNumber(${idx})" style="padding: 6px 12px; background: #f5576c; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 0.85rem; white-space: nowrap;">
                                                    <i class="fas fa-trash"></i> Delete
                                                </button>
                                            </div>
                                        </div>
                                        <div style="background: rgba(0,0,0,0.3); padding: 10px; border-radius: 6px; font-family: monospace; font-size: 0.9rem; margin-top: 10px; overflow-x: auto;">
                                            ${payment.isBank ? `
...
function savePaymentNumber() {
    ...
    authFetch('/api/payment-numbers/add', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
function deletePaymentNumber(idx) {
    if (confirm('Are you sure you want to delete this payment number?')) {
        authFetch(`/api/payment-numbers/delete/${idx}`, { method: 'DELETE' })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    showPaymentNumbersModal();
...
function saveWhatsAppAdmin() {
    ...
    authFetch('/api/whatsapp-admins/add', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
function deleteWhatsAppAdmin(idx) {
    if (confirm('Are you sure you want to remove this WhatsApp admin?')) {
        authFetch(`/api/whatsapp-admins/delete/${idx}`, { method: 'DELETE' })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    showWhatsAppAdminModal();
...
function showOrderLimitsModal() {
    const modal = `
        <div class="modal" onclick="closeModal(event)">
            <div class="modal-content" onclick="event.stopPropagation()" style="max-width: 600px;">
                <div class="modal-header">
                    <h2><i class="fas fa-sliders-h"></i> Order Limits Configuration</h2>
                    <button class="modal-close" onclick="closeModal()">&times;</button>
                </div>
                <div class="modal-body">
                    <div style="background: rgba(254, 202, 87, 0.1); border-left: 4px solid #feca57; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                        <p style="margin: 0; font-size: 0.95rem;"><strong>ℹ️ Note:</strong> These limits apply when users place diamond orders through WhatsApp.</p>
                    </div>
                    
                    <form id="orderLimitsForm">
                        <div style="margin-bottom: 20px;">
                            <label style="display: block; margin-bottom: 8px; font-weight: 600;">💎 Maximum Diamond Order Limit</label>
                            <p style="margin: 0 0 8px 0; font-size: 0.85rem; color: var(--text-secondary);">Users cannot order more diamonds than this limit in a single order</p>
                            <input type="number" id="maxDiamondLimit" style="width: 100%; padding: 12px; border-radius: 8px; background: var(--card-bg); color: var(--text-primary); border: 1px solid var(--border-color); font-size: 1rem;" placeholder="Enter maximum diamond limit" min="1000" max="999999" step="100">
                            <small style="display: block; margin-top: 5px; color: var(--text-secondary);">Current limit: <span id="currentMaxDiamond">10,000</span> diamonds</small>
                        </div>
                        
                        <div style="margin-bottom: 20px;">
                            <label style="display: block; margin-bottom: 8px; font-weight: 600;">💰 Maximum Deposit Amount Limit</label>
                            <p style="margin: 0 0 8px 0; font-size: 0.85rem; color: var(--text-secondary);">Users cannot deposit more than this amount in a single transaction</p>
                            <input type="number" id="maxDepositLimit" style="width: 100%; padding: 12px; border-radius: 8px; background: var(--card-bg); color: var(--text-primary); border: 1px solid var(--border-color); font-size: 1rem;" placeholder="Enter maximum deposit limit" min="10000" max="9999999" step="1000">
                            <small style="display: block; margin-top: 5px; color: var(--text-secondary);">Current limit: <span id="currentMaxDeposit">100,000</span> টাকা</small>
                        </div>
                        
                        <div style="background: rgba(102, 126, 234, 0.1); border: 1px solid rgba(102, 126, 234, 0.2); padding: 12px; border-radius: 8px; margin-bottom: 20px;">
                            <p style="margin: 0; font-size: 0.85rem;"><strong>📊 Statistics:</strong></p>
                            <p style="margin: 5px 0 0 0; font-size: 0.85rem; color: var(--text-secondary);">These limits help prevent fraud and control order sizes.</p>
                        </div>
                        
                        <div style="display: flex; gap: 10px;">
                            <button type="button" class="btn-primary" onclick="updateOrderLimits()" style="flex: 1;">
    ...
    // Load current limits
    authFetch('/api/order-limits')
        .then(r => r.json())
        .then(data => {
            if (data) {
                document.getElementById('maxDiamondLimit').value = data.maxDiamondOrder || 10000;
                document.getElementById('maxDepositLimit').value = data.maxDepositAmount || 100000;
                document.getElementById('currentMaxDiamond').textContent = (data.maxDiamondOrder || 10000).toLocaleString();
                document.getElementById('currentMaxDeposit').textContent = (data.maxDepositAmount || 100000).toLocaleString();
            }
        })
...
async function updateOrderLimits() {
    const maxDiamond = parseInt(document.getElementById('maxDiamondLimit').value);
    const maxDeposit = parseInt(document.getElementById('maxDepositLimit').value);
    
    if (isNaN(maxDiamond) || maxDiamond < 1000) {
        alert('❌ Diamond limit must be at least 1000');
        return;
    }
    
    if (isNaN(maxDeposit) || maxDeposit < 10000) {
        alert('❌ Deposit limit must be at least 10,000');
        return;
    }
    
    try {
        const response = await authFetch('/api/order-limits', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ maxDiamondOrder: maxDiamond, maxDepositAmount: maxDeposit })
        });
        
        const result = await response.json();
        if (result.success) {
            alert('✅ Order limits updated successfully!');
            closeModal();
            refreshAllData();
        } else {
        ...
        <div class="modal" onclick="closeModal(event)">
            <div class="modal-content" onclick="event.stopPropagation()">
                ...
                <div class="modal-body">
                    ...
                    <h3 style="margin-top: 25px;">ব্যবহারকারী ব্যবস্থাপনা</h3>
                    <div style="margin: 15px 0;">
                        <button class="btn-primary" onclick="closeModal(); showUserNameManagementModal()" style="width: 100%;">
                            <i class="fas fa-user-tag"></i> ব্যবহারকারীর নাম ব্যবস্থাপনা
                        </button>
                    </div>
...
// Show User Name Management Modal
function showUserNameManagementModal() {
    const modal = document.createElement('div');
    modal.className = 'modal';
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 700px;">
            <div class="modal-header">
                <h2><i class="fas fa-user-tag"></i> ব্যবহারকারীর নাম ব্যবস্থাপনা</h2>
            ...
            <div class="modal-body">
                ...
                <div class="form-section" style="margin-bottom: 30px;">
                    <h3 style="margin-bottom: 15px;">🆕 নতুন নাম যোগ করুন</h3>
                    <div style="display: grid; gap: 10px;">
                        <div>
                            <label style="font-size: 12px; color: #aaa; display: block; margin-bottom: 5px;">WhatsApp ID</label>
                            <input type="text" id="whatsappIdInput" placeholder="যেমন: 8732306071654@lid" 
                                   style="width: 100%; padding: 10px; border: 1px solid #2d3561; border-radius: 5px; background: #1a1a2e; color: #eee; font-family: monospace; font-size: 12px;">
                            <small style="color: #666; display: block; margin-top: 5px;">💡 টিপ: গ্রুপে অর্ডার দেওয়া সময় যে ID দেখা যায় সেটি কপি করুন</small>
                        </div>
                        
                        <div>
                            <label style="font-size: 12px; color: #aaa; display: block; margin-bottom: 5px;">প্রদর্শন নাম</label>
                            <input type="text" id="displayNameInput" placeholder="যেমন: আহমেদ, রহিম ইত্যাদি" 
                                   style="width: 100%; padding: 10px; border: 1px solid #2d3561; border-radius: 5px; background: #1a1a2e; color: #eee;">
                        </div>
                        
                        <button onclick="saveUserNameMapping()" style="padding: 10px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 5px; cursor: pointer; font-weight: 600;">
                ...
                <div class="form-section">
                    <h3 style="margin-bottom: 15px;">📋 বর্তমান নামগুলি</h3>
                    <div id="userNamesList" style="max-height: 400px; overflow-y: auto;">
...
// Load and display user name mappings
async function loadUserNameMappings() {
    try {
        const response = await fetch('/api/user-names/list');
        const data = await response.json();
        
        const listContainer = document.getElementById('userNamesList');
        if (!data.mappings || Object.keys(data.mappings).length === 0) {
            listContainer.innerHTML = '<p style="text-align: center; color: #666;">এখনো কোনো নাম যোগ করা হয়নি</p>';
            return;
        }
        
        let html = '<div style="display: grid; gap: 10px;">';
        for (const [whatsappId, displayName] of Object.entries(data.mappings)) {
            html += `
                <div style="display: flex; align-items: center; justify-content: space-between; background: rgba(102, 126, 234, 0.08); padding: 12px; border-radius: 5px; border: 1px solid rgba(102, 126, 234, 0.2);">
                    <div style="flex: 1;">
                        <div style="font-weight: 600; color: #eee;">${displayName}</div>
                        <div style="font-size: 11px; color: #666; font-family: monospace; margin-top: 5px;">${whatsappId}</div>
                    </div>
                    <button onclick="deleteUserNameMapping('${whatsappId}')" style="padding: 6px 12px; background: rgba(245, 87, 108, 0.2); color: #f5576c; border: 1px solid #f5576c; border-radius: 3px; cursor: pointer; font-size: 12px;">
        ...
        document.getElementById('userNamesList').innerHTML = '<p style="color: #f5576c;">ত্রুটি হয়েছে</p>';
    }
}

// Save a new user name mapping
async function saveUserNameMapping() {
    const whatsappId = document.getElementById('whatsappIdInput').value.trim();
    const displayName = document.getElementById('displayNameInput').value.trim();
    
    if (!whatsappId) {
        showToast('❌ WhatsApp ID লিখুন', 'error');
        return;
    }
    ...
    try {
        const response = await fetch('/api/user-names/save', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ whatsappId, displayName })
        });
        
        const data = await response.json();
        
        if (data.success) {
            showToast(`✅ সংরক্ষিত হয়েছে: ${displayName}`, 'success');
            document.getElementById('whatsappIdInput').value = '';
        ...
        showToast('❌ সংরক্ষণে ত্রুটি', 'error');
    }
}

// Delete a user name mapping
async function deleteUserNameMapping(whatsappId) {
    if (!confirm('এই নাম মুছে দেবেন?')) return;
    
    try {
        const response = await fetch(`/api/user-names/${encodeURIComponent(whatsappId)}`, {
            method: 'DELETE'
        });
        ...
        if (data.success) {
            showToast('✅ মুছে দেওয়া হয়েছে', 'success');
            loadUserNameMappings();
