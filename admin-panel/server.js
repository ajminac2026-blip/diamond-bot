...
const requireAuth = async (req, res, next) => {
    ...
    try {
        ...
        const validTokens = global.validTokens || new Set();
...
app.get('/', (req, res) => {
    // Always serve login page for root, let client-side handle redirect
...
// Serve main admin panel (protected route)
...
const userNamesMapPath = path.join(dbPath, 'user-names.json');
...
const adminCredentialsPath = path.join(adminPath, 'admin-credentials.json');

// In-memory token storage (use Redis in production)
global.validTokens = new Set();
...
app.get('/api/groups', requireAuth, async (req, res) => {
    try {
        ...
        const users = await readJSON(usersPath);
        const transactions = await readJSON(transactionsPath);
        const userNamesMap = await readJSON(userNamesMapPath).catch(() => ({})); // Load user names mapping
        const groups = database.groups || {};
        ...
        const groupsArray = Object.entries(groups).map(([id, data]) => {
            ...
            const userDueBreakdown = groupUsers.map(userId => {
                ...
                // 1. Check user-names.json manual mapping (highest priority)
                // 2. Check stored userName from orders (WhatsApp contact name - from msg pushname)
                // 3. Check userData.name
                // 4. Fallback to shortened userId
                let displayName = null;
                
                if (userNamesMap[userId]) {
                    // Manual mapping takes priority
                    displayName = userNamesMap[userId];
                } else if (userOrders.length > 0 && userOrders[0].userName && userOrders[0].userName !== userId) {
                    // Use WhatsApp contact name from first order if it's not just the userId
                    displayName = userOrders[0].userName;
                } else if (userData?.name) {
                    displayName = userData.name;
                }
                
                // If still no display name, use shortened ID
                if (!displayName) {
                    displayName = userId.substring(0, 15) + (userId.length > 15 ? '...' : '');
                }
            ...
            }); // Include all users, don't filter by due
...
// Update Group Rate
app.post('/api/groups/:groupId/rate', requireAuth, async (req, res) => {
    try {
...
app.post('/api/groups/:groupId/due-limit', requireAuth, async (req, res) => {
    try {
        const { groupId } = req.params;
...
// Clear Group Data
app.post('/api/groups/:groupId/clear', requireAuth, async (req, res) => {
    try {
        const { groupId } = req.params;
...
app.post('/api/groups/bulk-rate', requireAuth, async (req, res) => {
    try {
...
// Block/Unblock User
app.post('/api/users/:phone/toggle-block', requireAuth, async (req, res) => {
    try {
...
// Get Single User
app.get('/api/users/:phone', requireAuth, async (req, res) => {
    try {
        const { phone } = req.params;
        const users = await readJSON(usersPath);
        const database = await readJSON(databasePath);
...
app.post('/api/users/:phone/update', requireAuth, async (req, res) => {
    try {
        const { phone } = req.params;
...
app.post('/api/users/:phone/add-balance', requireAuth, async (req, res) => {
    try {
        const { phone } = req.params;
...
app.post('/api/users/:phone/add-due-balance', requireAuth, async (req, res) => {
    try {
        const { phone } = req.params;
        const { amount } = req.body;
        const users = await readJSON(usersPath);
...
// Approve Deposit
app.post('/api/deposits/:depositId/approve', requireAuth, async (req, res) => {
    try {
...
// Reject Deposit
app.post('/api/deposits/:depositId/reject', requireAuth, async (req, res) => {
    try {
        const { depositId } = req.params;
        const database = await readJSON(databasePath);
...
app.get('/api/orders', requireAuth, async (req, res) => {
    try {
        ...
        const ordersArray = Object.entries(orders).map(([id, data]) => ({
            id,
            phone: data.phone || 'Unknown',
            playerIdType: data.playerIdType || 'Unknown',
...
// Add Payment Number
app.post('/api/payment-numbers/add', requireAuth, async (req, res) => {
    try {
...
app.delete('/api/payment-numbers/delete/:index', requireAuth, async (req, res) => {
    try {
...
app.get('/api/whatsapp-admins', requireAuth, async (req, res) => {
    try {
...
// Add WhatsApp Admin
app.post('/api/whatsapp-admins/add', requireAuth, async (req, res) => {
    try {
...
app.delete('/api/whatsapp-admins/delete/:index', requireAuth, async (req, res) => {
    try {
        const index = parseInt(req.params.index);
        let admins = await readJSON(whatsappAdminsPath);
...
// Update admin WhatsApp ID (called by bot when admin sends message with unknown ID)
app.post('/api/whatsapp-admins/update-id', async (req, res) => {
    try {
        const { phone, whatsappId } = req.body;
        
        if (!phone || !whatsappId) {
            return res.status(400).json({ error: 'Phone and whatsappId are required' });
        }

        // Update bot's admins.json
        const botAdminsPath = path.join(__dirname, '..', 'config', 'admins.json');
        let botAdmins = [];
        try {
            const content = await fs.readFile(botAdminsPath, 'utf8');
        ...
        // Find and update the admin with this phone number
        const adminIndex = botAdmins.findIndex(a => a.phone === phone);
        if (adminIndex !== -1) {
            botAdmins[adminIndex].whatsappId = whatsappId;
            botAdmins[adminIndex].updatedAt = new Date().toISOString();
            await fs.writeFile(botAdminsPath, JSON.stringify(botAdmins, null, 2));
            
            // Log the update
            const logEntry = `[${new Date().toISOString()}] Updated admin ${phone} WhatsApp ID to ${whatsappId}`;
            const logsPath = path.join(__dirname, 'admin-logs.txt');
            const existingLogs = await fs.readFile(logsPath, 'utf8').catch(() => '');
            await fs.writeFile(logsPath, existingLogs + logEntry + '\n');
            
            console.log(`[ADMIN_UPDATE] Updated admin ${phone} with correct WhatsApp ID`);
            res.json({ success: true, message: 'Admin WhatsApp ID updated' });
        } else {
...
// Update user name in database entries (bulk operation)
app.post('/api/database/update-user-name', async (req, res) => {
    try {
        ...
        // Update all entries for this user across all groups
        Object.keys(database.groups || {}).forEach(groupId => {
            const group = database.groups[groupId];
            if (group.entries && Array.isArray(group.entries)) {
                group.entries.forEach(entry => {
                    if (entry.userId === userId && entry.userName !== newName) {
...
// Save user name mapping (WhatsApp ID → Display Name)
app.post('/api/user-names/save', requireAuth, async (req, res) => {
    try {
        const { whatsappId, displayName } = req.body;
        
        if (!whatsappId || !displayName) {
            return res.status(400).json({ 
                error: 'WhatsApp ID and display name are required' 
            });
        }
        ...
        let userNamesMap = {};
        try {
            userNamesMap = await readJSON(userNamesMapPath);
        } catch (e) {
            console.log('[USER-NAMES] Creating new user-names.json');
        }
        
        // Add or update the mapping
        userNamesMap[whatsappId] = displayName;
        
        // Save updated mapping
        await writeJSON(userNamesMapPath, userNamesMap);
        
        console.log(`[USER-NAMES] ✅ Saved mapping: ${whatsappId} → ${displayName}`);
        
        res.json({ 
            success: true, 
            message: `Name saved for ${whatsappId}`,
            whatsappId: whatsappId,
            displayName: displayName,
            totalMappings: Object.keys(userNamesMap).length
        });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// AUTO-SAVE user name mapping (called by bot when order is placed)
app.post('/api/user-names/auto-save', async (req, res) => {
    try {
        ...
        // Reject if displayName is just an ID-like string
        if (displayName.length <= 3 || displayName.includes('@')) {
            return res.status(400).json({ 
                error: 'Invalid display name (too short or looks like ID)',
        ...
        try {
            ...
            console.log('[USER-NAMES] Creating new user-names.json for auto-save');
        }
        
        // Only update if this user doesn't have a name yet
        if (userNamesMap[userId] && userNamesMap[userId] !== userId) {
            // Already has a manual name, don't override
            return res.json({ 
                success: false,
                message: `Already has manual name: ${userNamesMap[userId]}`,
                skipped: true
            });
        }
        ...
        console.log(`[USER-NAMES AUTO-SAVE] ✅ Auto-saved: ${userId} → ${displayName}`);
        
        res.json({ 
            success: true, 
            message: `Auto-saved name for ${userId}`,
            userId: userId,
            displayName: displayName,
            source: 'auto-save',
            totalMappings: Object.keys(userNamesMap).length
...
// Get all user name mappings
app.get('/api/user-names/list', requireAuth, async (req, res) => {
    try {
        let userNamesMap = {};
        try {
            userNamesMap = await readJSON(userNamesMapPath);
        } catch (e) {
            console.log('[USER-NAMES] No mappings found yet');
        }
        
        res.json({ 
            success: true,
            mappings: userNamesMap,
            totalMappings: Object.keys(userNamesMap).length
        });
    } catch (error) {
        res.status(500).json({ error: error.message });
...
// Delete user name mapping
app.delete('/api/user-names/:whatsappId', requireAuth, async (req, res) => {
    try {
        ...
        if (!userNamesMap[whatsappId]) {
            return res.status(404).json({ 
                error: `Mapping not found for ${whatsappId}` 
            });
        }
        
        delete userNamesMap[whatsappId];
        await writeJSON(userNamesMapPath, userNamesMap);
        
        console.log(`[USER-NAMES] ✅ Deleted mapping for ${whatsappId}`);
        
        res.json({ 
            success: true,
            message: `Mapping deleted for ${whatsappId}`,
            totalMappings: Object.keys(userNamesMap).length
        });
    } catch (error) {
        res.status(500).json({ error: error.message });
...
// DEBUG: Get all users from all groups (for debugging name issues)
app.get('/api/debug/all-users', requireAuth, async (req, res) => {
    try {
        const database = await readJSON(databasePath);
        const userNamesMap = await readJSON(userNamesMapPath).catch(() => ({}));
        ...
        Object.entries(groups).forEach(([groupId, groupData]) => {
            (groupData.entries || []).forEach(entry => {
                if (!allUsers.has(entry.userId)) {
                    allUsers.set(entry.userId, {
                        userId: entry.userId,
                        storedUserName: entry.userName,
                        mappedName: userNamesMap[entry.userId] || null,
                        status: 'found'
                    });
                }
            });
        ...
        const result = Array.from(allUsers.values()).map(user => {
            let displayName = user.mappedName;
            if (!displayName && user.storedUserName && user.storedUserName !== user.userId) {
                displayName = user.storedUserName;
            }
            if (!displayName) {
                displayName = user.userId.substring(0, 15) + '...';
            }
            
            return {
                ...user,
                displayName: displayName,
                nameSource: user.mappedName ? 'manual-mapping' : (user.storedUserName && user.storedUserName !== user.userId ? 'stored-name' : 'auto-shortened-id')
            };
        });
        
        res.json({
            success: true,
            totalUniquUsers: result.length,
            users: result,
            mappingsCount: Object.keys(userNamesMap).length
        });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.get('/api/auto-deductions/last', requireAuth, async (req, res) => {
    try {
...
app.get('/api/auto-deductions', requireAuth, async (req, res) => {
    try {
        const deductions = await readJSON(autoDeductionsPath);
        const database = await readJSON(databasePath);
        const groups = database.groups || {};
...
app.post('/api/auto-deductions', requireAuth, async (req, res) => {
    try {
...
app.delete('/api/auto-deductions', requireAuth, async (req, res) => {
    try {
...
// Clear All Data
app.post('/api/clear-all-data', requireAuth, async (req, res) => {
    try {
        const { pin } = req.body;
...
// Send Due Reminders
app.post('/api/send-due-reminders', requireAuth, async (req, res) => {
    try {
...
app.get('/api/diamond-status', requireAuth, async (req, res) => {
    try {
...
app.post('/api/diamond-status/toggle', requireAuth, async (req, res) => {
    try {
...
app.post('/api/approve-message/toggle', requireAuth, async (req, res) => {
    try {
        let status = await readJSON(diamondStatusPath);
...
app.post('/api/diamond-status/global-message', requireAuth, async (req, res) => {
    try {
        let status = await readJSON(diamondStatusPath);
...
app.post('/api/diamond-status/group-message', requireAuth, async (req, res) => {
    try {
...
// Set Diamond Stock
app.post('/api/diamond-status/set-stock', requireAuth, async (req, res) => {
    try {
...
// Order Limits configuration endpoints
const orderLimitsPath = path.join(dbPath, 'order-limits.json');

// Initialize order limits if not exists
async function initOrderLimits() {
    try {
        await readJSON(orderLimitsPath);
    } catch (e) {
        const defaults = {
            maxDiamondOrder: 10000,
            maxDepositAmount: 100000,
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
        };
        await writeJSON(orderLimitsPath, defaults);
    }
}

// Get current order limits
app.get('/api/order-limits', requireAuth, async (req, res) => {
    try {
        await initOrderLimits();
        const limits = await readJSON(orderLimitsPath);
        res.json(limits);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.post('/api/order-limits', requireAuth, async (req, res) => {
    try {
        const { maxDiamondOrder, maxDepositAmount } = req.body;
        
        if (!maxDiamondOrder || !maxDepositAmount) {
            return res.status(400).json({ 
                success: false, 
                message: 'Missing required fields: maxDiamondOrder, maxDepositAmount' 
            });
        }
        
        if (maxDiamondOrder < 1000) {
            return res.status(400).json({ 
                success: false, 
                message: 'Diamond limit must be at least 1000' 
            });
        }
        
        if (maxDepositAmount < 10000) {
            return res.status(400).json({ 
                success: false, 
                message: 'Deposit limit must be at least 10,000' 
            });
        }
        ...
        await writeJSON(orderLimitsPath, limits);
        
        console.log(`[ORDER-LIMITS] Updated: Diamond=${maxDiamondOrder}, Deposit=${maxDepositAmount}`);
