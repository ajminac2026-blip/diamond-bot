...
// Add diamond entry
function addEntry(groupId, userId, diamonds, rate, groupName, messageId, userName) {
...
function isAdmin(whatsappId) {
    ...
    const exactMatch = admins.find(a => a.whatsappId === whatsappId);
    if (exactMatch) {
        return true;
    }
    
    // If no exact match, check if this might be an admin whose true WhatsApp ID we haven't captured
    // Look for admins with placeholder IDs (format: phone@lid where phone contains digits and @)
    const idNumeric = whatsappId.split('@')[0];
    const potentialAdmin = admins.find(a => {
        if (!a.whatsappId || !a.phone) return false;
        
        // Check if admin's stored ID looks like a placeholder (phone@lid format)
        const storedIdNumeric = a.whatsappId.split('@')[0];
        
        // If stored ID matches phone pattern, it's likely a placeholder
        // (e.g., "8801927628504@lid" is a placeholder, "39874946191383@lid" is real)
        if (storedIdNumeric === a.phone && a.phone.length > 10) {
            // This is a placeholder. But we can't assume the new ID belongs to this admin
            // without verification. However, we can notify admin panel to check.
            return false;
        }
        return false;
    });
